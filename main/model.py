...
class RNNBaseline(BaseModel):
    def __init__(self, timesteps, features, n_classes, model_name="test_model", rnn_type='simpleRNN',
        ...
        self.model.add(Masking(mask_value=0.0, input_shape=(self.timesteps, self.features), name="mask"))
        if self.rnn_type == 'simpleRNN':
            self.model.add(
                SimpleRNN(n_units, input_shape=(self.timesteps, self.features), return_sequences=True,activation=activation, name="rnn"))
        
        if self.rnn_type == 'LSTM':
            self.model.add(
                LSTM(n_units, input_shape=(self.timesteps, self.features), return_sequences=True,activation=activation, name="lstm"))
...
class RNNBXModel(BaseModel):
    def __init__(self, timesteps, features, n_classes, model_name="test_model", rnn_type='simpleRNN',
                 loss='categorical_crossentropy', metrics=[], activation="relu", n_units=20,cv_features=0,cv_units=2,cv_activation="linear",cv_bias=False):
        BaseModel.__init__(self, n_classes, model_name=model_name, rnn_type=rnn_type, loss=loss,
                          metrics=metrics)
        # initialize RNN model
        self.timesteps = timesteps
        self.features = features
        model1 = Sequential()
        model1.add(Masking(mask_value=0.0, input_shape=(self.timesteps, self.features), name="mask"))
        if self.rnn_type == 'simpleRNN':
            model1.add(
                SimpleRNN(n_units, input_shape=(self.timesteps, self.features), return_sequences=True,activation=activation, name="rnn"))
        ...
        context_model.add(Dense(output_dim=cv_units,input_shape=(timesteps,cv_features),name='external-context',activation=cv_activation,bias=cv_bias))
        print "context_model summary:"
        print context_model.summary()
                
        self.model=Sequential()
        self.model.add(Merge([model1, context_model], mode='concat'))
        self.model.add(TimeDistributed(Dense(n_classes, activation='softmax'), name="output"))
        self.model.compile(loss=self.loss, optimizer='adam', metrics=[self.loss] + self.metrics)
...
class RNNY2YModel(BaseModel):
    def __init__(self, timesteps, x_dim, y_dim, model_name="y_to_y_model", rnn_type='simpleRNN',
                 loss='categorical_crossentropy', metrics=[], z_to_z_activation="relu", z_dim=20,y_to_y_activation="linear",xz_to_y_activation="linear",y_bias=False,xz_bias=False):
