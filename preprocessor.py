...
class Preprocessor():
    def __init__(self, vocab, pad_value=0., seq_length=None,sparse=False):
        self.vocab = vocab
...
class FullModelPreprocessor(Preprocessor):
    def __init__(self, vocab, pad_value=0., seq_length=None,sparse=False):
        Preprocessor.__init__(self, vocab, pad_value, seq_length, sparse=sparse)
    ...
    def transform_data(self, sequences, xs, pad=True):
        ...
        if not self.sparse:
            ...
            features_dim = 1
        padded_x_data = self._pad_sequences(x_data,dtype=np.float64)
        padded_c_data = self._pad_sequences(c_data,dtype=np.float64)
        padded_y_data = self._pad_sequences(y_data,dtype=np.float64)
        padded_y_data = np.reshape(padded_y_data, (len(padded_y_data), self.seq_length, features_dim))
        padded_c_data = np.reshape(padded_c_data, (len(padded_c_data), self.seq_length, len(self.vocab)))
        padded_x_data = np.reshape(padded_x_data, (len(padded_x_data), self.seq_length, features_dim))
