...
def _data_statistics(data_table,group_col,seq_col):
    ...
    basic_stats = pd.DataFrame([counts_by_user[seq_col].min(), counts_by_user[seq_col].max(), counts_by_user[seq_col].median(), counts_by_user[seq_col].mean()], \
...
def load_flickr_data():
    """Load Flickr dataset.
        ...
        """ 
    data_table=_read_data("../data/","traj-noloop-all-Melb.csv",delimiter=",",skipinitialspace=True)
    return data_table
