...
class BaseModel():
    ...
    def fit_model(self, x_train, y_train, x_val, y_val, n_epochs=10, batch_size=100, plot_history=False, verbose=1):
        history = self.model.fit(x_train, y_train, validation_data=(x_val, ys_val), epochs=n_epochs,
...
class RNNBaseline(BaseModel):
    def __init__(self, timesteps, features, n_classes, model_name="baseline_model", rnn_type='simpleRNN',
        ...
        if self.rnn_type == 'simpleRNN':
            self.model.add(
                SimpleRNN(n_units, input_shape=(timesteps, features), return_sequences=True, activation=activation,
                          name="rnn"))

        if self.rnn_type == 'LSTM':
            self.model.add(
                LSTM(n_units, input_shape=(timesteps, features), return_sequences=True, activation=activation,
                     name="lstm"))
...
class RNNY2YModel(BaseModel):
    def __init__(self, timesteps, x_dim, y_dim, model_name="y_to_y_model", rnn_type='simpleRNN',
                 loss='categorical_crossentropy', metrics=[], x_to_y=False, z_to_z_activation="relu", z_dim=20,
                 y_to_y_activation="linear", xz_to_y_activation="linear", y_bias=False, xz_bias=False):
        ...
        if self.rnn_type == 'simpleRNN':
            rnn_model.add(
                SimpleRNN(z_dim, input_shape=(timesteps, x_dim), return_sequences=True, activation=z_to_z_activation,
                          name="rnn-x-to-z"))

        if self.rnn_type == 'LSTM':
            rnn_model.add(
                LSTM(z_dim, input_shape=(timesteps, x_dim), return_sequences=True, activation=z_to_z_activation,
                     name="lstm-x-to-z"))
        ...
        if x_to_y:
            ...
            xz_output = TimeDistributed(Dense(x_dim, activation=xz_to_y_activation, bias=xz_bias), name="xz_output")(
                xz_input)
